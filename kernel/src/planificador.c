...
void iniciar_planificador(void) {
    ...
    if (strcmp(algoritmo_plani, "FIFO") == 0 || strcmp(algoritmo_plani, "PRIORIDADES") == 0) {
...
void planific_corto_multinivel_rr(void) {
    ...
    while (true) {
        for (int nivel = 0; nivel < 10000; nivel++) {
            ...
            switch (codigo_recibido) {
                ...
                case INTERRUPCION: // Este sería el caso donde se interrumpe por fin de quantum
                log_debug(log_kernel_gral, "%d:%d - Desalojado por fin de Quantum.", hilo_exec->pid_pertenencia, hilo_exec->tid);
